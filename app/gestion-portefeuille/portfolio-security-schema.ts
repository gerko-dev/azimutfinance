...
// ============================================================
// Complétude d'un titre saisi à la main
// ============================================================
//
// Un titre créé depuis un inventaire non reconnu n'hérite d'aucun paramètre du
// référentiel de marché : tout se saisit. Rien n'oblige à tout remplir sur le
// moment — mais certains champs ne sont pas décoratifs, et leur absence casse
// une fonction précise du module.
//
// La liste ci-dessous n'est donc PAS un jugement sur ce qui serait « bien
// renseigné » : chaque champ y figure parce qu'un axe d'allocation ou un calcul
// le consomme. Le commentaire dit lequel, pour qu'on puisse discuter de son
// caractère requis plutôt que de l'imposer.

export type ChampRequis = { key: string; motif: string };

/** Provenances qui désignent réellement une référence du référentiel de marché.
 *
 *  `dat` et `cash` sont des natures de position, pas des sources : un dépôt à
 *  terme ou un compte bancaire n'existe dans aucun référentiel. `custom`
 *  renvoie à un titre déjà local. Les traiter comme des liens produisait des
 *  comptes de trésorerie badgés « Lié au site » dont la référence était par
 *  construction introuvable. */
export const SOURCES_LIEES: ReadonlySet<string> = new Set([
  "stock",
  "listed-bond",
  "sovereign",
  "fund",
...
// ============================================================
//
// Un même titre ne porte pas le même libellé d'un export à l'autre :
//
//   « FCTC SENELEC 10% 2025-2030 »            (export sans symboles)
//   « FCTC SENELEC MEZZANINE 10 %2025-2030 »  (référentiel)
//
// Le rapprochement par nom exact ne peut rien contre cet écart, et renommer le
// titre ne règle rien : le libellé change encore au prochain export — un DAT
// « reconduit au 08.09.26 » devient « au 08.10.26 ».
//
// On mémorise donc TOUS les libellés sous lesquels un titre a déjà été
// rapproché. Le gérant rattache une fois ; les imports suivants reconnaissent,
// quel que soit le libellé employé.

/** Clé d'alias : nom normalisé, réduit à [a-z0-9 ]. */
export function cleAlias(s: string): string {
  return (s ?? "")
    .normalize("NFD")
    .replace(/[̀-ͯ]/g, "")
    .toLowerCase()
...
/** Séparateur des alias stockés.
 *
 *  Sûr par construction : les clés étant normalisées à [a-z0-9 ], une barre
 *  verticale ne peut jamais apparaître DANS une clé. Un séparateur choisi
 *  parmi les caractères possibles d'un libellé produirait des alias coupés en
 *  deux au premier titre qui le contient. */
const SEPARATEUR_ALIAS = "|";

/** Alias enregistrés sur un titre. */
export function lireAlias(attributes: Record<string, string> | undefined): string[] {
  return (attributes?.alias ?? "")
    .split(SEPARATEUR_ALIAS)
    .map((s) => s.trim())
    .filter((s) => s.length >= 4);
}

/** Ajoute un libellé aux alias, sans doublon. Renvoie null si rien ne change —
 *  l'appelant évite ainsi une écriture inutile en base. */
export function ajouterAlias(
  attributes: Record<string, string> | undefined,
  libelle: string,
  dejaConnus: string[] = [],
): string | null {
  const cle = cleAlias(libelle);
  if (cle.length < 4) return null;
  const existants = lireAlias(attributes);
  const connus = new Set([...existants, ...dejaConnus.map(cleAlias)]);
  if (connus.has(cle)) return null;
  return [...existants, cle].join(SEPARATEUR_ALIAS);
}

/** Le titre est-il rattaché à une référence du site exploitable ? */
export function estLieAuSite(attributes: Record<string, string> | undefined): boolean {
  const source = (attributes?.source ?? "").trim();
  const refId = (attributes?.refId ?? "").trim();
  return !!refId && SOURCES_LIEES.has(source);
}

export const CHAMPS_REQUIS: Record<PortfolioSection, ChampRequis[]> = {
  action: [
    { key: "secteur", motif: "axe d'allocation Actions — par secteur" },
  ],
  obligation: [
    { key: "issuerType", motif: "axe Obligations — par émetteur (État, privé, régional)" },
    { key: "country", motif: "rattachement à l'État émetteur" },
    { key: "maturityDate", motif: "axe Obligations — par maturité résiduelle" },
    { key: "couponRate", motif: "coupon moyen pondéré et valorisation" },
    { key: "nominalValue", motif: "cours en % du nominal" },
    {
      key: "amortizationMode",
      motif:
        "neutralisation de l'amortissement — en mode N le cours suit le nominal, en mode T il ne bouge pas",
    },
  ],
  opcvm: [
    { key: "categorie", motif: "ventilation par classe d'actif" },
  ],
  dat: [
    { key: "dateEcheance", motif: "maturité résiduelle et échéancier de trésorerie" },
    { key: "tauxInteret", motif: "intérêts courus" },
    { key: "montantNominal", motif: "valorisation" },
  ],
  tresorerie: [
    { key: "pays", motif: "concentration par pays" },
    { key: "banque", motif: "concentration par établissement" },
  ],
  autre: [],
};

/** Champs requis non renseignés, pour un titre donné. Liste vide = complet. */
export function champsManquants(
  kind: PortfolioSection,
  attributes: Record<string, string> | undefined,
): ChampRequis[] {
  const attrs = attributes ?? {};
  return (CHAMPS_REQUIS[kind] ?? []).filter((c) => {
    const v = (attrs[c.key] ?? "").toString().trim();
    return v === "";
  });
}

/** Libellé d'un champ, pour afficher ce qui manque sans exposer la clé brute. */
export function libelleChamp(kind: PortfolioSection, key: string): string {
  return SECURITY_FIELDS[kind]?.find((f) => f.key === key)?.label ?? key;
}
