...
export default function ProDividendsTab({
  ticker,
  currentPrice,
  ratios,
}: Props) {
    ...
    <div className="space-y-4">
      ...
      <section className="bg-slate-800/40 border border-slate-700 rounded-lg p-4">
        <div className="flex items-baseline justify-between flex-wrap gap-2 mb-3">
          <h3 className="text-base font-medium text-white">Historique des dividendes</h3>
        ...
        <div className="mt-4 overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-xs text-slate-400 bg-slate-900/60 border-y border-slate-700">
            ...
            <tbody>
              {[...history].reverse().map((h) => (
                <tr key={h.exercice} className="border-b border-slate-800">
                  <td className="px-3 py-2 font-medium text-slate-200">{h.exercice}</td>
                  ...
                  <td className={`px-3 py-2 text-right font-mono ${pctColor(h.growth)}`}>
                    {formatPctSigned(h.growth, 1)}
                  </td>
                  <td className="px-3 py-2 text-right font-mono text-slate-200">
                    {h.yieldPct !== null ? formatPct(h.yieldPct, 2) : "—"}
                  </td>
                  <td className="px-3 py-2 text-right font-mono text-slate-200 hidden md:table-cell">
      ...
      <section className="bg-slate-800/40 border border-slate-700 rounded-lg p-4">
        <div className="flex items-baseline justify-between flex-wrap gap-2 mb-4">
          <div>
            <h3 className="text-base font-medium text-white">Simulateur de revenus</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-5">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 text-sm bg-slate-900 border border-slate-700 rounded-md text-slate-200 placeholder:text-slate-500 focus:outline-none focus:border-blue-500"
            />
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 text-sm bg-slate-900 border border-slate-700 rounded-md text-slate-200 placeholder:text-slate-500 focus:outline-none focus:border-blue-500"
            />
            <div className="text-[11px] text-slate-500 mt-1">Entre 1 et 20 ans</div>
          </div>
              ...
              <tbody>
                {simRows.map((r) => (
                  <tr key={r.year} className="border-b border-slate-800">
                    <td className="px-3 py-2 font-medium text-slate-200">N+{r.year}</td>
                    <td className="px-3 py-2 text-right font-mono text-slate-200">
      ...
      {projection.length > 0 && (
        <section className="bg-slate-800/40 border border-slate-700 rounded-lg p-4">
          <div className="flex items-baseline justify-between flex-wrap gap-2 mb-3">
            <div>
              <h3 className="text-base font-medium text-white">Projection DPA · 5 ans</h3>
              <p className="text-xs text-slate-500 mt-0.5">
...
function ScoreCard({
  ...
  const palette: Record<typeof score.level, { grad: string; text: string; label: string }> = {
    excellent: {
      grad: "from-emerald-500/15 border-emerald-500/40",
      text: "text-emerald-300",
      label: "Excellent",
    },
    bon: {
      grad: "from-blue-500/15 border-blue-500/40",
      text: "text-blue-300",
      label: "Bon",
    },
    moyen: {
      grad: "from-amber-500/15 border-amber-500/40",
      text: "text-amber-300",
      label: "Moyen",
    },
    faible: {
      grad: "from-red-500/15 border-red-500/40",
      text: "text-red-300",
      label: "Faible",
    },
  };
  ...
  return (
    <div
      className={`bg-gradient-to-br ${p.grad} to-slate-900/20 rounded-lg border p-4`}
    >
