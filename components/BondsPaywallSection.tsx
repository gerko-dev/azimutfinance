...
type Props = {
  ...
  description: string;
  /** Liste de bullets décrivant ce qu'on obtient avec Premium. */
  features: string[];
  /** Si false, l'utilisateur n'est pas connecté → CTA différent. Défaut: true. */
  isMember: boolean;
};

/**
 * Paywall plein page pour les sections Premium (courbe / surveiller / calendrier).
 * Server component pur — pas de state. Le CTA pointe vers /compte (upgrade) ou
 * /connexion selon que l'utilisateur est connecté ou non.
 */
export default function BondsPaywallSection({
  breadcrumb,
  title,
  description,
  features,
