...
import type { CommoditySlug, ReturnHorizon } from "@/lib/commodities";
...
function correlationColor(v: number | null): string {
  ...
  // Diverging palette : -1 rouge, 0 blanc, +1 bleu
  if (v >= 0) {
...
export default function CommoditiesAnalyzer({
  initialPeriod,
  stats,
  normalizedByPeriod,
  correlationByPeriod,
  ...
  stats: StatRow[];
  /** Series base 100 pre-calculees serveur pour CHAQUE periode possible */
  normalizedByPeriod: Record<Period, NormalizedSnapshot>;
  correlationByPeriod: Record<Period, CorrelationSnapshot>;
  defaultSelection: CommoditySlug[];
}) {
  const [period, setPeriod] = useState<Period>(initialPeriod);
  const [selected, setSelected] = useState<Set<CommoditySlug>>(
  ...
  function toggle(slug: CommoditySlug) {
    setSelected((prev) => {
  ...
  // Tableau perf trie par perf YTD
  const sortedStats = useMemo(() => {
    return [...stats].sort((a, b) => (b.returns.YTD ?? -Infinity) - (a.returns.YTD ?? -Infinity));
  }, [stats]);
    ...
    <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
        ...
        <div className="flex items-center justify-between gap-3 mb-3 flex-wrap">
          <div>
            <h2 className="text-base md:text-lg font-semibold text-slate-900">
              Studio matières premières
            </h2>
            <p className="text-[11px] text-slate-500 mt-0.5">
              Comparez, corrélez et mesurez la pression macro sur la BRVM. 8 sous-jacents · maille
              quotidienne.
            </p>
          </div>
      ...
      <div className="px-4 md:px-6 py-4 border-b border-slate-200">
            ...
            >
              ...
              <Tooltip
                ...
                formatter={(v, name) => {
                  ...
                  if (typeof v !== "number") return ["—", meta?.name ?? String(name)];
                  return [`${v.toFixed(1).replace(".", ",")}  (${(v - 100).toFixed(1).replace(".", ",")} %)`, meta?.name ?? String(name)];
                }}
              />
              <Legend
                wrapperStyle={{ fontSize: 11 }}
                iconSize={8}
                formatter={(value) => stats.find((s) => s.slug === value)?.name ?? value}
              />
        ...
        <p className="text-[10px] text-slate-400 mt-1">
          Lecture : valeur à 100 = prix au début de la fenêtre. Une valeur à 130 signifie +30 % depuis cette date.
        </p>
      </div>
      ...
      <div className="px-4 md:px-6 py-4 border-b border-slate-200 overflow-x-auto">
        ...
        <table className="w-full text-xs border-separate border-spacing-0">
          <thead>
            <tr className="text-slate-500 text-[10px] uppercase">
              <th className="text-left font-medium py-2 pr-3 sticky left-0 bg-white">Sous-jacent</th>
              ...
              {RETURN_HORIZONS.map((h) => (
                <th key={h.id} className="text-right font-medium py-2 px-2">
                  {h.label}
                </th>
              ))}
              ...
              <tr key={s.slug} className="border-t border-slate-100 hover:bg-slate-50">
                ...
                {RETURN_HORIZONS.map((h) => {
                  ...
                  return (
                    <td
                      key={h.id}
                      className={`text-right tabular-nums py-1.5 px-2 ${
                        v === null
                          ? "text-slate-300"
                          : v >= 0
                          ? "text-emerald-600"
        ...
        <p className="text-[10px] text-slate-400 mt-2">
          Lecture du z-score : écart entre le cours actuel et la moyenne 1 an, exprimé en écarts-types. |z| &gt; 2 = niveau extrême.
        </p>
      </div>
              ...
              <thead>
                <tr>
                  ...
                  {filteredCorrelation.labels.map((l) => (
                    <th
                      key={l}
                      className="p-1 text-slate-500 font-medium text-center align-bottom"
                      style={{ minWidth: 64 }}
                    >
